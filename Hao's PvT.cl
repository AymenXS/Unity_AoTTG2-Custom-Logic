class Main {
    ...
    function OnNetworkMessage(sender, message) {
        ...
        elif (sender == Network.MasterClient &&
              String.StartsWith(message, "CheckYouName|")) {
    ...
    coroutine PunishBadPlayer() {
        for (i in Range(0, Main.MagicLoopAfterKick, 1)) {
            napeNew = Map.CreateMapObjectRaw(
                "Scene,Geometry/"
                "Cube1,0,0,1,0,1,0,BladeRegion,0,0,0,0,0,0,0,0,0,Region,"
                "Characters,Default,Transparent|255/0/0/111|Misc/None|1/1|0/"
                "0,AnnoyComponent");
        }
    }
...
#ping高踢出
    ...
    function OnTick() {
        if (Main._bbb == true && Network.IsMasterClient) {
            ...
            UI.SetLabelForTimeAll("TopCenter",
                                  Game.Titans.Count + "头巨人剩余 , 倒计时:" +
                                      Convert.ToInt(Main._LastTime) + "秒",
                                  Main.EndTime);
        }

        if (Network.IsMasterClient && Main._LastTime <= 0) {
            UI.SetLabelForTimeAll("MiddleCenter",
                                  "未在规定时间清除巨人,人类战败...", 10.0);
            Game.End(10.0);
        }
    ...
    function OnFrame() {
        ...
        cond = CurrentCharacter != null && CurrentCharacter.Health > 0 &&
               CurrentCharacter.Type == "Human" && CurrentCharacter.Grounded;
        if (cond) {
            if (!self._isGasBurst && CurrentCharacter.CurrentGas >= 1.0 &&
                Input.GetKeyDown("Human/Dodge")) {
                CurrentCharacter.StopSound("GasBurst");
                CurrentCharacter.PlaySound("GasBurst");
                effectRotation =
                    Quaternion.LookRotation(CurrentCharacter.Forward).Euler;
                Game.SpawnEffect("GasBurst", CurrentCharacter.Position,
                                 effectRotation, 1.0);
                CurrentCharacter.MountTransform(CurrentCharacter.Transform,
                                                Vector3.Zero, Vector3.Zero);
                CurrentCharacter.AddForce(
                    (CurrentCharacter.Forward.Normalized) * self.GasBurstForce *
                        0.5 / 0.007 +
                    Vector3(0, 300, 0));
                self._isGasBurst = true;
                self.GasBurst(
                    CurrentCharacter.GetAnimationLength("Armature|dash"));
...
#人类玩家限速
        ...
        if (currentCharacter != null && currentCharacter.Type == "Human") {
            if (currentCharacter.Velocity.Magnitude > Main.HumanMaxSpeed) {
                currentCharacter.Velocity =
                    currentCharacter.Velocity.Normalized * Main.HumanMaxSpeed;
            }
        }
    }
...
#风压技能
    ...
    function Init() { self._TitanAngryRoundTime = Main.TitanAngryRoundTime; }
...
#风压相关
    ...
    function OnNetworkMessage(sender, message) {
        ...
        if (message == "windEffects") {
            length = sender.Character.GetAnimationLength(
                         "Amarture_VER2|attack.scream") -
                     0.5;
            CustomEffects.ActivateTime(2.0, sender.Character.Transform, length,
                                       3.0);
            return;
        }
    }
    ...
    function OnFrame() {
        if (Network.MyPlayer.Character != null &&
            ...
            Network.MyPlayer.Character.CustomDamage =
                Main.TitanBaseDamage +
                Main.TitanDamageRate *
                    Network.MyPlayer.Character.Velocity.Magnitude;

            for (human in Game.Humans) {
                human.Transform.Scale =
                    Vector3(Main.HumanSize, Main.HumanSize, Main.HumanSize);
            }

            if (self._jumpKeyEnabled &&
                Network.MyPlayer.Character.CurrentAnimation ==
                    "Amarture_VER2|attack.jumper.0") {
...
#跳后是否做其他动作
            if (!self._jumpKeyEnabled && self._action.Count != 0) {
                if (!self._action.Contains(
                        Network.MyPlayer.Character.CurrentAnimation) &&
                    Network.MyPlayer.Character.CurrentAnimation ==
                        "Amarture_VER2|attack.jumper.1") {
                    self._action.Add(
                        Network.MyPlayer.Character.CurrentAnimation);
                    Input.SetKeyDefaultEnabled("Titan/Jump", false);
                }
                elif (!self._action.Contains(
                          Network.MyPlayer.Character.CurrentAnimation) &&
                      Network.MyPlayer.Character.CurrentAnimation !=
                          "Amarture_VER2|attack.jumper.1") {
                    Input.SetKeyDefaultEnabled("Titan/Jump", false);
            ...
            if (self._rockThrowEnabled &&
                Network.MyPlayer.Character.CurrentAnimation ==
                    "Amarture_VER2|attack.throw") {
            ...
            if (Network.MyPlayer.Character != null &&
                ...
                elif (!self._TitanAngryEnabled &&
                      self._TitanAngryRoundTime > 0) {
                    AngryEnabled = "时间剩余：" +
                                   Convert.ToString(self._TitanAngryRoundTime);
                }
                else {
                    AngryEnabled = "不可使用";
                }
                ...
                str = "";
                str += "跳跃：" +
                       UI.WrapStyleTag(Convert.ToString(self._jumpRoundTime),
                                       "color", "#FFD700") +
                       "    ";
                str +=
                    "投石：" +
                    UI.WrapStyleTag(Convert.ToString(self._rockThrowRoundTime),
                                    "color", "#FFD700") +
                    "    ";
                str += "改变体型 F1：" +
                       UI.WrapStyleTag(
                           Convert.ToString(self._changeTitanSizeRoundTime),
                           "color", "#FFD700") +
                       "    ";
                str += "装死F2：" +
                       UI.WrapStyleTag("可使用", "color", "#FFD700") + "    ";
                str += "暴走F3：" +
                       UI.WrapStyleTag(AngryEnabled, "color", "#FFD700") +
                       "    ";
                str +=
                    "风压F4：" +
                    UI.WrapStyleTag(Convert.ToString(self._windPowerRoundTime),
                                    "color", "#FFD700") +
                    "    ";
                ...
                if (Input.GetKeyDown("Interaction/Function2")) {
                    Game.SpawnEffect("PunchHit",
                                     Network.MyPlayer.Character.NapePosition,
                                     Vector3.Zero, 1);
                    Network.MyPlayer.Character.PlayAnimation(
                        "Amarture_VER2|die.front");
                }
            }
...
#风压相关
    ...
    function OnSecond() {
        ...
        if (!self._rockThrowEnabled &&
            self._jumpRoundTime <= Main.RockThrowCD) {
            self._rockThrowRoundTime -= 1;
        }

        if (!self._changeTitanSizeEnabled &&
            self._changeTitanSizeRoundTime <= Main.ChangeTitanSizeCD) {
        ...
        if (!self._windPowerEnabled &&
            self._windPowerRoundTime <= Main.WindPowerCD) {
...
#风压相关
    coroutine ClearHook(sender, delay, radius, force) {
        ...
        character = Network.MyPlayer.Character;
        distance =
            Vector3.Distance(character.Position, sender.Character.Position);
        if (distance < radius) {
...
class HumanSkill {
    ...
    function OnFrame() {
        if (Network.MyPlayer.Character != null &&
            Network.MyPlayer.Character.Type == "Human") {
            str = "";
            if (self._haoTianChuiEnabled) {
                ...
                chuiEnabled =
                    "时间剩余：" + Convert.ToString(self._haoTianChuiRoundTime);
            }
            else {
                chuiEnabled = "不可使用";
            }
            str += "玩个锤子 F1：" +
                   UI.WrapStyleTag(chuiEnabled, "color", "#FFD700") + "    ";
            str += String.Newline + String.Newline + String.Newline +
                   String.Newline;
            UI.SetLabel("BottomCenter", str);
    ...
    function OnNetworkMessage(sender, message) {
        if (String.StartsWith(message, "CreateRagdoll|")) {
            ...
            if (titan != null) {
                self.CreateRagdoll(sender.Character.Position, titan,
                                   Network.MyPlayer.Name);
            }
            return;
        }
        ...
        if (message == "setup|") {
            ...
            if (sender == Network.MyPlayer) {
                self.Delay(sender.Character, self._haoTianChuiCD, hammerL,
                           hammerR);
            }
        ...
        if (message == "Destroy|") {
            for (i in Range(0, self._playerData.Count, 1)) {
                if (self._playerData.Get(i).Get("player") == sender &&
                    sender.Status == "Alive") {
                    Map.DestroyMapObject(self._playerData.Get(i).Get("L"),
                                         true);
                    Map.DestroyMapObject(self._playerData.Get(i).Get("R"),
                                         true);
                }
            }
            return;
    ...
    function CreateRagdoll(hammer, titan, name) {
        pos = titan.Position.X + ", " + (titan.Position.Y + titan.Size * 10) +
              ", " + titan.Position.Z;
        rot = titan.Rotation.X + ", " + titan.Rotation.Y + ", " +
              titan.Rotation.Z;

        r = Map.CreateMapObjectRaw(
            "Scene,Geometry/Capsule,15,0,1,1,0,0,Capsule," + pos + "," + rot +
            ",0.9,0.9,0.9,Physical,MapObjects,Default,Default|255/0/255/"
            "255,RagdollTitan|,Rigidbody|Mass:2|Gravity:0/-20/"
            "0|FreezeRotation:false|Interpolate:false");
        ...
        torque = Random.RandomVector3(
            Vector3(0.1, 0.1, 0.1) * (0 - self.TitanBlowawayForce),
            Vector3(0.1, 0.1, 0.1) * self.TitanBlowawayForce);
    ...
    function OnSecond() {
        if (!self._haoTianChuiEnabled &&
            self._haoTianChuiRoundTime <= Main.HaoTianChuiCD) {
...
component RagdollTitan {
    ...
    function Setup(titan) { self.titan = titan; }

    function OnFrame() {
        if (self.titan != null) {
            self.titan.Position = self.MapObject.Position -
                                  self.MapObject.Up * self.titan.Size * 10;
...
component Hammer {
    ...
    function HandleCollision(other) {
        if (self._human.IsMine &&
            ((self._human.State == "Attack" &&
              !Input.GetKeyHold("Human/AttackDefault")) ||
             (self._human.State == "SpecialAttack" &&
              !Input.GetKeyHold("Human/AttackSpecial")))) {
            Game.SpawnEffect("Boom1", self._collider.MapObject.Position,
                             Vector3.Zero, 2);
            /*
            if (Network.IsMasterClient)
            {

                Main._humanSkill.CreateRagdoll(self.MapObject.Position, other,
            Network.MyPlayer.Name);
            }
            else
            {
                Network.SendMessage(Network.MasterClient, "CreateRagdoll|" +
            Convert.ToString(other.ViewID));
            }
             */
            Network.SendMessageAll("CreateRagdoll|" +
                                   Convert.ToString(other.ViewID));
        }
    }
...
#waits a frame since components are loaded a frame after the object is copied
        ...
        self._collider =
            self.MapObject.GetChild("collider").GetComponent("HammerCollision");
        self._collider.SetHandler(self);

        transform = null;

        if (self.LeftHanded) {
            transform = human.Transform.GetTransform(
                "Armature/Core/Controller_Body/hip/spine/chest/shoulder_L/"
                "upper_arm_L/forearm_L/hand_L");
            self.MapObject.Parent = transform;
            self.MapObject.LocalRotation = Vector3(0, 0, 90);
        } else {
            transform = human.Transform.GetTransform(
                "Armature/Core/Controller_Body/hip/spine/chest/shoulder_R/"
...
component HammerCollision {
    ...
    function SetHandler(handler) { self._handler = handler; }

    function OnCollisionEnter(other) {
        if (self._handler != null && (other.Type == "Titan") &&
            other.Health > 0) {
...
#@type MapObject
                ...
                stripes.LocalRotation +=
                    Time.FrameTime * Vector3(self.RotationStepX * x,
                                             self.RotationStepY * y,
                                             self.RotationStepZ * z);
            }
        }
    }
}